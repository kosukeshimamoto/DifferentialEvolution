...
function run_trial(f, lower, upper, rng; algorithm, popsize, maxevals, F, CR, pmax, target, parallel)
...
function run_benchmark()
    dim = parse(Int, get(ENV, "DE_PAR_DELAY_DIM", "50"))
    runs = parse(Int, get(ENV, "DE_PAR_DELAY_RUNS", "1"))
    base_seed = parse(Int, get(ENV, "DE_PAR_DELAY_SEED", "2025"))
    target_tol = parse(Float64, get(ENV, "DE_PAR_DELAY_TARGET_TOL", "1e-2"))
    de_F = parse(Float64, get(ENV, "DE_PAR_DELAY_F", "0.5"))
    de_CR = parse(Float64, get(ENV, "DE_PAR_DELAY_CR", "0.9"))
    pmax = parse(Float64, get(ENV, "DE_PAR_DELAY_PMAX", "0.25"))
    popsize = parse(Int, get(ENV, "DE_PAR_DELAY_POPSZ", "50"))
    maxevals = parse(Int, get(ENV, "DE_PAR_DELAY_MAXEVALS", "400"))
    delays = parse_float_list("DE_PAR_DELAY_DELAYS", [1e-2])

    outdir = joinpath(pwd(), "reports", "benchmarks", "parallel_compare_delay")
    mkpath(outdir)
    ...
    open(md_path, "w") do io
        println(io, "# Parallel compare (delayed eval) benchmark summary")
        println(io)
        ...
        println(io, "- modes: serial (parallel=false), parallel (parallel=true)")
        println(io, "- threads: ", nthreads())
        println(io, "- delay targets (s): ", join(delays, ", "))
        println(io, "- base function: Rastrigin")
        println(io)

        println(io, "| Delay (s) | Algorithm | Mode | Runs | Maxevals | Popsize | Threads | Time mean (s) | Time std (s) |")
        println(io, "| --- | --- | --- | --- | --- | --- | --- | --- | --- |")
        for delay in delays
    ...
    open(csv_path, "w") do io
        println(io, "delay_s,dim,algorithm,mode,runs,maxevals,popsize,threads,time_mean_s,time_std_s")
        for row in results
            println(
                io,
                format_float(row.delay_s), ",",
