...
function run_de_trial(f, lower, upper, rng; algorithm, popsize, maxevals, F, CR, pmax, target, mode)
...
function run_benchmark()
    maxevals = parse(Int, get(ENV, "DE_ACC_MAXEVALS", "20000"))
    dims = parse_int_list("DE_ACC_DIMS", [10, 30, 60])
    runs = parse(Int, get(ENV, "DE_ACC_RUNS", "3"))
    base_seed = parse(Int, get(ENV, "DE_ACC_SEED", "2025"))
    target_tol = parse(Float64, get(ENV, "DE_ACC_TARGET_TOL", "1e-8"))
    de_F = parse(Float64, get(ENV, "DE_ACC_F", "0.5"))
    de_CR = parse(Float64, get(ENV, "DE_ACC_CR", "0.9"))
    pmax = parse(Float64, get(ENV, "DE_ACC_PMAX", "0.25"))
    heavy_iters = parse(Int, get(ENV, "DE_ACC_HEAVY_ITERS", "0"))

    outdir = joinpath(pwd(), "reports", "benchmarks", "accuracy_parallel")
    mkpath(outdir)
    ...
    for dim in dims
        ...
        for (mode_label, mode_sym) in modes
            for (alg_index, (alg_label, alg_id)) in enumerate(algorithms)
                ...
                for (task_index, task) in enumerate(tasks)
                    ...
                    final_errors_dim = Float64[]
                    times_list = Float64[]

                    for run in 1:runs
                        seed_alg = base_seed + run + 1000 * alg_index + 100000 * config_id + 10 * dim + 1000000 * task_index
                        ...
                        push!(final_errors, abs(best_f - task.optimum))
                        push!(final_errors_dim, abs(best_f - task.optimum) / dim)
                        push!(times_list, elapsed)
                    end

                    err_stats = mean_std(final_errors)
                    err_dim_stats = mean_std(final_errors_dim)
                    ...
                    push!(results, (
                        ...
                        error_dim_mean=err_dim_stats.mean,
                        error_dim_std=err_dim_stats.std,
    ...
    open(md_path, "w") do io
        ...
        println(io, "- error metric: mean(|f - f*|) and mean(|f - f*|)/dim")
        println(io)

        for task in tasks
            println(io, "## ", task.name)
            println(io)
            println(io, "| Dim | Algorithm | Mode | Runs | Maxevals | Popsize | Threads | Error mean | Error std | Error mean / dim | Error std / dim | Time mean (s) | Time std (s) |")
            println(io, "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |")
            for dim in dims
                for (alg_label, _) in algorithms
                    for (mode_label, _) in modes
                        ...
                        println(
                            ...
                            format_float(row.error_dim_mean), " | ",
                            format_float(row.error_dim_std), " | ",
    ...
    open(csv_path, "w") do io
        println(io, "task,dim,algorithm,mode,runs,maxevals,popsize,threads,error_mean,error_std,error_dim_mean,error_dim_std,time_mean_s,time_std_s")
        for row in results
            println(
                io,
                row.task, ",",
